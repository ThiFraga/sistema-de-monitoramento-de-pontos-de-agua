...
class PontosDB {
    ...
    insert = database.transaction((doc: IPontos) => {
        const query = this.insertSQL();
        doc.id = randomUUID();
        query.run(doc);
    });

    list(options: IOptions) {
        const params: unknown[] = [];
        let query = "SELECT * FROM pontos_coleta WHERE 1 = 1"
        
        if(!!options.filters) {
            
            Object.keys(options.filters).map((key: string) => {
                query += ` AND ${key} = ?`;
                params.push(options.filters![key]);
            });
        }
        if (!!options.sort) {
            Object.keys(options.sort).map((key: string) => {
    ...
    update = database.transaction((id: string, doc: Partial<IPontos>) => {
    ...
    delete = database.transaction((id: string) => {
        const query = database.prepare(`
